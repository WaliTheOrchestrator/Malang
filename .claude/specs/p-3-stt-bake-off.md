...
| `corpus/p3/README.md` + manifest | The corpus layout and provenance: which WAV is which register, the hand-corrected reference transcripts, and the proper-noun tags. **The reference transcripts + manifest are committed; the raw voice WAVs are git-ignored and kept locally** (30 min of the owner's voice is personal data, and the record's sacredness lives in `memory/`, not in test fixtures — but committing his voice to the code repo is a privacy choice made deliberately, not by default). Confirm this split in review. |
...
Every ASR runtime here is CPU-first by intent, but **CPU-only + Windows must be confirmed by `docs-researcher` before this spec is approved** — §11's rule: an unknown in these columns is a research task, not an implementation-day discovery. The most promising single-library path is **`onnx-asr`**, which claims unified ONNX CPU support for Moonshine, Parakeet, *and* Whisper — if confirmed on Windows it collapses three deps into one; otherwise fall back to per-model runtimes.

| Package | Version | Why | CPU-only? | Windows? | License |
|---|---|---|---|---|---|
| `onnx-asr` (candidate, preferred) | pin at install | Unified onnxruntime CPU loader for Moonshine + Parakeet TDT + Whisper — one dep for the whole bake-off if it holds up | **NEEDS RESEARCH** (onnxruntime CPU is the whole point; confirm all three model families load) | **NEEDS RESEARCH** (win_amd64 wheel + no espeak-style native surprise) | **NEEDS RESEARCH** |
| Moonshine ONNX runtime (fallback) | pin at install | Live-path candidate, small+medium, if `onnx-asr` doesn't cover it | **NEEDS RESEARCH** | **NEEDS RESEARCH** | **NEEDS RESEARCH** |
| Parakeet TDT 0.6B v3 runtime (fallback) | pin at install | Memory-path default; avoid the full NeMo stack on this machine if an ONNX/CT2 build exists | **NEEDS RESEARCH** (NeMo is heavy; want an ONNX/CT2 build) | **NEEDS RESEARCH** | **NEEDS RESEARCH** |
| `faster-whisper` (or whisper via `onnx-asr`) | pin at install | large-v3-turbo int8 second-pass challenger; CTranslate2 int8 is the "int8/CT2" the spec names | **NEEDS RESEARCH** (CT2 CPU int8 is well-trodden; confirm the -turbo weights + win wheel) | **NEEDS RESEARCH** | **NEEDS RESEARCH** |
| `jiwer` | pin at install | Standard WER/CER computation; keeps the metric off hand-rolled code | likely yes (pure-Python) — **confirm** | likely yes — **confirm** | Apache-2.0 — **confirm** |

Model weights (Moonshine small+medium, Parakeet TDT 0.6B v3, whisper-large-v3-turbo CT2 int8) are **manual, one-time downloads** scripted like P-2's — pinned URLs/revisions recorded in `corpus/p3/README.md`, licenses checked at the source repo, not from PyPI metadata. **Nothing is installed against the system 3.14 interpreter** — the `uv` venv stays pinned to 3.12 (the P-2 lesson). These deps are needed anyway by `speech/stt_final.py` (M7) and `speech/stt_live.py` (M3a), so P-3 introduces nothing the project wasn't already taking.
...
- **Manual (the measurement itself):**
  1. Record ~30 min on the **real mic**: monologue + a read passage + a deliberately code-switched passage, including "Malang" and family names spoken naturally. Hand-correct into reference transcripts; tag proper nouns and code-switch spans.
